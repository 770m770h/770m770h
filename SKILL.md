...
1. **Never promise "safe".** This is static analysis + reputation. Always give the caveat
   and separate "what I scanned in the code" (facts) from "what is a reputation signal"
   (inference).
...
**5. Present the shortlist** per the verdict template (table: # · name · what it does ·
...
**3. Give the final verdict** per the verdict template: risk band + label, the **code
...
**4. Scoring meaning** lives in the scoring rubric; the detection rules in the threat model. Read
them if unsure how a number was reached.
...
- If a source failed or returned nothing, say so - don't imply full coverage.
...
- Sources are config in `references/sources.yaml`. The skills.sh audit is a cross-check, never sole
  truth.
